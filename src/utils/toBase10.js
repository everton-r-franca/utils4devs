...
function toBase10(convert) {
	...
	if (!messages.hasErrors()) {
		const digits = greater9Converter(
			extractDigits(numberStringSanitized)
		);
...
function allSanitizeNumber(numberString, base) {
	...
	if (!numberIsValid(numberStringSanitized))
		return messages.addError(
			"O deve ter no mínimo 1 digito e no máximo 1M"
		);
...
function conversorAlgorithm(digits, base) {
	...
	for (let i = 0; i < digitsLength; i++) {
		...
		steps.push(
			`${digits[digitsIteration]}x${base}^${i} = ${bigInt(
				digits[digitsIteration]
			).multiply(bigInt(base).pow(i))}`
		);
		digitsIteration--;
	}
	console.log(steps);
...
function greater9Converter(digits) {
	...
	return digits.map((digit) => {
		...
		return parseInt(
			number >= a && number <= f ? number - correction : digit
		);
	});
}
